...
!$acc update device(gamma_min, pres_inf)
        ...
        if (barotropic) then
            @:ALLOCATE(rho_igr(ix%beg:ix%end, iy%beg:iy%end), dux_igr(ix%beg:ix%end, iy%beg:iy%end), dvx_igr(ix%beg:ix%end, iy%beg:iy%end))
...
!!$acc parallel loop collapse(3) gang vector default(present)
            ...
            if (id == 1) then
                ...
                do k = iy%beg + 1, iy%end - 1
                    do j = ix%beg+1, ix%end-1
                        dux_igr(j, k) = (1/(2d0*dx(j))) * ( &
                            q_prim_qp%vf(momxb)%sf(j+1,k,0) - &
                            q_prim_qp%vf(momxb)%sf(j-1,k,0) ) 

                        duy_igr(j,k) = (1/(2d0*dy(k))) * ( &
                            q_prim_qp%vf(momxb)%sf(j,k+1,0) - &
                            q_prim_qp%vf(momxb)%sf(j,k-1,0) ) 
                    end do
                end do
                ...
                do k = iy%beg + 1, iy%end - 1
                    do j = ix%beg+1, ix%end-1
                        dvx_igr(j, k) = (1/(2d0*dx(j))) * ( &
                            q_prim_qp%vf(momxb + 1)%sf(j+1,k,0) - &
                            q_prim_qp%vf(momxb + 1)%sf(j-1,k,0) ) 

                        dvy_igr(j,k) =  (1/(2d0*dy(k))) * ( &
                            q_prim_qp%vf(momxb + 1)%sf(j,k+1,0) - &
                            q_prim_qp%vf(momxb + 1)%sf(j,k-1,0) ) 
                    end do
                end do
                ...
                do k = iy%beg + 1, iy%end - 1
                    do j = ix%beg + 1, ix%end - 1
                        ...
                        rhs_igr(4, j, k) = rho_igr(j, k) *alf_igr  *2d0 * (dvx_igr(j, k)*duy_igr(j, k) + dvy_igr(j, k)**2d0)
                    end do
                end do
                ...
                do k = 0, n
                    do j = 0, m
                        ...
                        fd_coeff(j,k) = rho_igr(j,k) / alf_igr
                        ...
                        fd_coeff(j,k) = fd_coeff(j,k) + (1d0 / dx(j)**2d0) * (rho_lx + rho_rx) +  (1d0 / dy(k)**2d0) *(rho_ly + rho_ry) 
                    ...
                    do q = 1, 2
                        do k = 0, n
                            do j = 0, m
                                ...
                                jac_igr(q, j, k) = jac_rhs_igr(q, j, k)
                ...
                do k = iy%beg , iy%end - 1
                    do j = ix%beg , ix%end - 1                        
                        ...
                        F_igr(4, j, k) = 1d0* (rhs_igr(4, j, k) - rho_igr(j,k)*(1d0 / dy(k))*(jac_igr(2,j,k+1) - jac_igr(2,j,k)))
                    end do
                end do 
                ...
                do i = 1, 3
                    do k = 0, n
                        do j = 0, m
                            ...
                            if(bcxb < -1) then
                                rhs_vf(i)%sf(0,k,0) = 0d0
                            end if

                            if(bcxe < -1) then
                                rhs_vf(i)%sf(m,k,0) = 0d0
                            end if

                            if(bcyb < -1) then
                                rhs_vf(i)%sf(j,0,0) = 0d0
                            end if

                            if(bcye < -1) then
                                rhs_vf(i)%sf(j,n,0) = 0d0
                            end if
                ...
                do i = 1, 3
                    do k = 0, n
                        do j = 0, m
                            ...
                            rhs_vf(i)%sf(j,k,0) = &
                                rhs_vf(i)%sf(j,k,0) + 1d0/(2d0*dy(k)) * &
