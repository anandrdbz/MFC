        ...
        if (barotropic) then
            ...
            do l = iy%beg, iy%end
                do j = ix%beg, ix%end
                   do i = 1, 2
                        jac_igr(i, j, 0) = 0d0
                        jac_old_igr(i, j, 0) = 0d0
                        jac_rhs_igr(i, j, 0) = 0d0
                   end do
                end do
            end do
    ...
    !$acc parallel loop collapse(3) gang vector default(present)
            ...
            if (id == 1) then
                ...
                if(barotropic) then
                    ...
                    do k = iy%beg + 1, iy%end - 1
                        do j = ix%beg + 1, ix%end - 1
                            rhs_igr(1, j, k) = (dux_igr(j, k)**2d0 + duy_igr(j, k) * dvx_igr(j, k))
                            rhs_igr(2, j, k) = (dux_igr(j, k)*duy_igr(j, k) + duy_igr(j, k) * dvy_igr(j, k))                        
                            rhs_igr(3, j, k) = (dvx_igr(j, k)*dux_igr(j, k) + dvy_igr(j, k) * dvx_igr(j, k))
                            rhs_igr(4, j, k) = (dvx_igr(j, k)*duy_igr(j, k) + dvy_igr(j, k)**2d0)
                        end do
                    end do
                    ...
                    do k = iy%beg + 1, iy%end - 1
                        do j = ix%beg + 1, ix%end - 1
                            jac_rhs_igr(1, j, k) = alf_igr * (rhs_igr(1, j, k) + rhs_igr(4, j, k)  + (dux_igr(j, k) + dvy_igr(j, k))**2d0 )
                        end do
                    end do
                    ...
                    do k = 0, n
                        do j = 0, m
                            rho_lx = 0.5d0 *(1d0 / rho_igr(j,k) + 1d0 / rho_igr(j-1,k)) 
                            rho_rx = 0.5d0 *(1d0 / rho_igr(j,k) + 1d0 / rho_igr(j+1,k)) 
                            rho_ly = 0.5d0 *(1d0 / rho_igr(j,k) + 1d0 / rho_igr(j,k-1)) 
                            rho_ry = 0.5d0 *(1d0 / rho_igr(j,k) + 1d0 / rho_igr(j,k+1))

                            ! rho_lx = 1d0 / rho_igr(j-1, k)
                            ! rho_rx = 1d0 / rho_igr(j, k)
                            ! rho_ly = 1d0 / rho_igr(j, k-1)
                            ! rho_ry = 1d0 / rho_igr(j, k)

                            fd_coeff(j,k) = 1d0 / rho_igr(j, k)
                            ...
                            fd_coeff(j,k) = fd_coeff(j,k) + alf_igr * ( (1d0 / dx(j)**2d0) * (rho_lx + rho_rx) +  (1d0 / dy(k)**2d0) *(rho_ly + rho_ry) )
                        ...
                        do q = 1, 1
                            do k = 0, n
                                do j = 0, m
                                    ...
                                    jac_igr(q,j,k) = jac_igr(q,j,k) + alf_igr * (1d0 / dx(j)**2d0) * (rho_lx* jac_old_igr(q,j-1,k) + rho_rx*jac_old_igr(q,j+1,k)) &
                                            + alf_igr * (1d0 / dy(k)**2d0) * (rho_ly* jac_old_igr(q,j,k-1) + rho_ry* jac_old_igr(q,j,k+1)) 
