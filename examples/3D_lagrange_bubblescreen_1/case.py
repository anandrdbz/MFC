...
# Count bubbles from bubbles.txt
bubbles_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "input/lag_bubbles.dat")
nBubs = 0
bub_radii = []
with open(bubbles_file, "r") as f:
    for line in f:
        ...
        if not line or line.startswith("#"):
            continue
        nBubs += 1
        bub_radii.append(float(line.split()[3]))
print(f"Found {nBubs} bubbles in {bubbles_file}", file=__import__('sys').stderr)
